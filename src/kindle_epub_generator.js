...
class KindleEPUBMagazineGenerator {
    constructor(kindleOptimized = true) {
        this.zip = new JSZip();
        this.chapters = [];
        this.images = [];
        ...
        this.kindleOptimized = kindleOptimized;
    }
    ...
    initializeEPUB(title, author, description) {
        ...
        // Add CSS - use Kindle-optimized styles if enabled
        let cssContent = '';
        try {
            const cssPath = this.kindleOptimized 
                ? path.join(path.dirname(fileURLToPath(import.meta.url)), "kindle_epub_styles.css")
                : path.join(path.dirname(fileURLToPath(import.meta.url)), "epub_styles.css");
            
            cssContent = readFileSync(cssPath, "utf-8");
        } catch (error) {
            console.error(`Warning: Could not read '${this.kindleOptimized ? 'kindle_epub_styles.css' : 'epub_styles.css'}'. Proceeding without custom styles. Error: ${error.message}`);
